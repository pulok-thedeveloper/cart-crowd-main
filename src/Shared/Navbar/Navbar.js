...
const Navbar = () => {
    const {categories, menProducts, womenProducts} =useContext(AuthContext);
    ...
    return (
        <div className='nav-bar grid grid-cols-4 px-16'>
            <div className='logo grid items-center justify-start'>
                <Link to='/' className='logo-title'>CartCrowd</Link>
            </div>
            <div className='nav-menu grid items-center justify-center col-span-2'>
                <ul className='h-full flex items-center gap-10'>
                    ...
                    <li id='shop' className='h-full flex items-center relative'>
                        <Link className='h-full flex items-center' to='/shop'>SHOP <IoIosArrowDown className='ml-2'/></Link>
                        <div className='dropdown-menu absolute top-full text-left p-10'>
                            <p className='mb-5'>PRODUCT CATEGORIES</p>
                            <ul className='grid grid-cols-2 gap-4'>
                                {
                                    categories?.map(category => <li key={category._id}><Link className='hover-underline' to={`/shop/${category.name}`}>{category.name}</Link></li>)
                                }
                            </ul>
                        </div>
                    </li>
                    <li id='men' className='h-full flex items-center relative'>
                        <Link className='h-full flex items-center' to='/shop/Men'>MEN <IoIosArrowDown className='ml-2'/></Link>
                        <div className='dropdown-menu absolute top-full text-left p-10'>
                            <p className='mb-5'>MEN'S FASHION</p>
                            <ul className='grid grid-cols-2 gap-4'>
                                {
                                    menProducts?.map(category => <li key={category._id}><Link className='hover-underline' to={`Men/${category.name}`}>{category.name}</Link></li>)
                                }
                            </ul>
                        </div>
                    </li>
                    <li id='women' className='h-full flex items-center relative'>
                        <Link className='h-full flex items-center' to='/shop/Women'>WOMEN <IoIosArrowDown className='ml-2'/></Link>
                        <div className='dropdown-menu absolute top-full text-left p-10'>
                            <p className='mb-5'>WOMEN'S FASHION</p>
                            <ul className='grid grid-cols-2 gap-4'>
                                {
                                    womenProducts?.map(category => <li key={category._id}><Link className='hover-underline' to={`/Women/${category.name}`}>{category.name}</Link></li>)
                                }
                            </ul>
                        </div>
                    </li>
            ...
            <div className='nav-right grid items-center justify-end'>
                <ul className='flex gap-10'>
                    ...
                    <li><FaRegUser /></li>
                    <li className='cursor-pointer' onClick={()=>setSearchModal(true)}><BsSearch /></li>
                </ul>
            </div>
            {
                searchModal && 
